...
const useScrollToSection = () => {
  ...
  const handleScrollToSection = () => {
    if (contentRef.current) {
      ...
      const topPosition = contentRef.current.getBoundingClientRect().top + window.scrollY - offset
    ...
    <>
      ...
      <div className="container px-0 md:px-6 grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div className="lg:col-span-1">
          <WhitepaperSidebar activeSection={activeSection} onSectionChange={onSectionChange} />
        </div>
        <div className="lg:col-span-3">
          <WhitepaperContent activeSection={activeSection} onSectionChange={onSectionChange} contentRef={contentRef} />
        </div>
      </div>
    </>
  )
