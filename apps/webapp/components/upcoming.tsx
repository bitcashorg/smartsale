      ...
      <AnimatePresence>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 scroll-m-3" key="upcoming-auctions-description-container">
          {isUpcomingCardsContainerVisible && projects.map((item, index) => (
            <motion.div key={`upcoming-auctions-item-${index}`} {...auctionCardAnimationProps(index)}>
              <AuctionCard {...item} />
            </motion.div>
          ))}
        </div>
...
const auctionCardAnimationProps: (index: number) => MotionProps = (index: number) => ({
  initial: { opacity: 0, y: -40 },
  animate: { opacity: 1, y: 0 },
  exit: { opacity: 0, y: 40 },
  transition: { type: 'spring', bounce: 0.4, duration: 0.75, delay: (index + 1) * 0.62 },
})
