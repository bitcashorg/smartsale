...
async function handleNewAuction(log: NewAuctionEvent) {
  ...
  const auctioningToken = await getTokenDetails({
    address: log.args._auctioningToken,
  })
  const biddingToken = await getTokenDetails({
    address: log.args._biddingToken,
  })
  ...
  const data = {
    ...
    order_cancellation_end_date: bigintToPostgresTimestamp(
      log.args.orderCancellationEndDate,
    ),
