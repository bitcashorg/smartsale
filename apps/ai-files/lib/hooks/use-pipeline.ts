import { type Pipeline, type PretrainedOptions, Tensor } from '@xenova/transformers'
import { useEffect, useState } from 'react'
import type { InitEventData, OutgoingEventData, RunEventData } from '../workers/pipeline'
...
export function usePipeline(task: string, model?: string, options?: PretrainedOptions) {
  ...
  useEffect(() => {
    ...
    const worker = new Worker(new URL('../workers/pipeline.ts', import.meta.url), {
      type: 'module',
    })
