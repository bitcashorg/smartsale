...
export const renderEmailByPath = async (
  ...
  const { emailComponent: Email, createElement, render, sourceMapToOriginalFile } = result
  ...
  try {
    ...
    const plainText = await render(createElement(EmailComponent, previewProps), {
      plainText: true,
    })
    ...
    return {
      error: improveErrorWithSourceMap(error, emailPath, sourceMapToOriginalFile),
    }
  }
}
